...
file_prefix = f"python-server-{key_label}-sha{hash_bits}"
# construction
if key_type == "rsa":
    server_key = rsa.generate_private_key(public_exponent=65537, key_size=key_size)
elif key_type == "ec":
    server_key = ec.generate_private_key(curve)  # 預設為 P-256
else:
...
# === 8. 使用 CA 私鑰簽章 ===
server_cert = builder.sign(private_key=ca_key, algorithm=hash_algorithm)
